...
const RelatedProducts:React.FC = () => {
  ...
  return (
    <div>
        <h2 className="my-6 md:my-0 text-center text-3xl font-bold font-playfair text-textColor-heading">Related Products</h2>
            ...
            {relatedProducts.slice().reverse().map((product) => (
                <div key={product.ID} className="md:my-8 w-[276px] h-[388px] border border-gray-200 rounded-lg shadow-md overflow-hidden relative group">
                    ...
                    <div className="relative p-5 transition-transform duration-300 ease-in-out group-hover:translate-y-[-40px] mx-2 rounded-lg bg-background-default cursor-pointer">
                        <Link to={`/product/${product.ID}`}><h4 className="text-textColor-subheading font-playfair text-lg">{product.NAME}</h4>
                            <div className="flex gap-2 my-5 text-sm text-textColor-default opacity-0 group-hover:opacity-100 transition-opacity duration-300">
